...
# Fit some models
mod0<-lm(mpg~1, data=mtcars) #intercept only model
summary(mod0)
...
# write_csv(mtcars, file="./data/mtcars_raw.csv")

cars_temp<-read.csv(file="./data/untidy/mtcars_raw.csv")

cars_temp%>%
  group_by(cyl)%>%
  summarize(avg=mean(mpg), sd=sd(mpg))%>%
  arrange(sd) %>%
  write.csv(file="./data/tidy/mtcars_tidy.csv")

read.csv(file="./data/tidy/mtcars_tidy.csv")
